...
> [!TIP]
> To deploy this project using **GUI-based flow**, navigate to [console](https://console.stacktape.com/create-new-project/git-project-using-console?name=my-stacktape-app&repositoryType=public&repositoryUrl=https://github.com/stacktape/starter-discord-bot)

- [Discord bot](https://discord.com/developers/docs) running as a
  [worker service](https://docs.stacktape.com/compute-resources/worker-services/) on AWS.
- Uses [discord.js](https://discord.js.org/) with a persistent WebSocket connection to Discord's gateway.
- Always-on container — ideal for real-time bot interactions, slash commands, and event handling.
...
```bash
stacktape init --starterId discord-bot
```
...
- Before deploying, create a bot at the [Discord Developer Portal](https://discord.com/developers/applications).
- Set the `discord-bot-token` secret to your bot token (or update the default in `stacktape.yml`).
- After deploying, invite the bot to your server using the OAuth2 URL from the Developer Portal (with `bot` and
...
```bash
stacktape dev --region <<your-region>> --stage <<stage>> --resourceName bot
```
